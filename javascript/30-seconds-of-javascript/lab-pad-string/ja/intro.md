...
この実験では、JavaScript の `pad()` 関数を調べます。`pad()` 関数を使うと、文字列を両端に指定した文字で埋めて、特定の長さにすることができます。これを達成するために、`String.prototype.padStart()` と `String.prototype.padEnd()` メソッドをどのように使うか学びます。
