...
カンマ区切り値 (CSV) の文字列をオブジェクトの2次元配列に変換してコーディングの練習に使うには、ターミナル/SSH を開いて `node` と入力します。文字列の最初の行はタイトル行として使用されます。CSV を JSON に変換する手順は以下の通りです。
...
6. デフォルトの区切り文字として `,` を使うために、2番目の引数 `delimiter` を省略します。
