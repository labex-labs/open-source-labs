...
上記の例では、`expandTabs`関数は2つの引数をとります。タブを含む文字列`str`と、各タブ文字を置き換える半角スペースの数を表す数値`count`です。この関数は、正規表現(`/\t/g`)を使った`String.prototype.replace()`メソッドを使って、入力文字列内のすべてのタブ文字を見つけ、`String.prototype.repeat()`メソッドを使って必要な数の半角スペースに置き換えます。
