...
5. 2番目の引数 `id` を省略すると、既定で `null` になります。これはオブジェクトが別のオブジェクトにリンクされていないことを示します（つまり、トップレベルのオブジェクトです）。
6. 3番目の引数 `link` を省略すると、既定のプロパティとして `'parent_id'` が使われます。これにより、オブジェクトがその `id` によって別のオブジェクトにリンクされます。
