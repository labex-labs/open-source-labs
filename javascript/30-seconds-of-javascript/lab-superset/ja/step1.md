...
- `Array.prototype.every()` と `Set.prototype.has()` を使用して、2 番目の反復可能オブジェクトの各値が最初の反復可能オブジェクトに含まれていることを確認します。
- 最初の反復可能オブジェクトが 2 番目の反復可能オブジェクトのスーパーセットであり、重複する値を除外した場合、関数は `true` を返します。それ以外の場合は `false` を返します。
