...
Для отображения изображения с надписью используйте элементы `<figure>` и `<figcaption>`. В CSS используйте свойство `linear-gradient`, чтобы создать эффект наложения над изображением. Вот пример кода:
