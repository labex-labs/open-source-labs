...
To create a pretty text underline that does not clip descenders:

- In `index.html` and `style.css`, use `text-shadow` to apply 4 values with offsets covering a 4x4 `px` area. This ensures the underline has a thick shadow that covers the line where descenders clip it. For best results, use a color that matches the `background` and adjust the `px` values for larger fonts.
...
- **Note:** This effect is natively implemented as `text-decoration-skip-ink: auto`, but it has less control over the underline.
