...
1. `<svg>`要素の中に`<symbol>`要素を持つチェックマークを作成します。`<use>`要素を使って、再利用可能な SVG アイコンとして挿入します。
2. `.checkbox-container`クラスを持つコンテナ div を作成します。Flexbox を使って、チェックボックスに適切なレイアウトを作成します。
3. `<input>`要素を非表示にし、チェックボックスと提供されたテキストを表示するためにラベルを関連付けます。
4. 状態変化時にチェックマークにアニメーションを追加するには、`stroke-dashoffset`を使います。
5. ズームアニメーションエフェクトを作成するには、CSS アニメーションを通じて`transform: scale(0.9)`を使います。
