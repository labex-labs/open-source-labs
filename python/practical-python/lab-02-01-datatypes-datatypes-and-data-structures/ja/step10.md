...
ここ数回のエクササイズでは、`portfolio.csv` というデータファイルを読み込むプログラムを書きました。`csv` モジュールを使えば、ファイルを1行ずつ読み込むことが簡単です。
...
さらに多くのことを行うには、通常、生のデータを何らかの方法で解釈し、後で使えるようにもっと便利な種類のオブジェクトに変換する必要があります。2つの簡単なオプションはタプルまたは辞書です。
