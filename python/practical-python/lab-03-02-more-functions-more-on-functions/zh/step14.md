...
在许多情况下，你只对CSV文件中的某些特定列感兴趣，而不是所有数据。修改 `parse_csv()` 函数，使其可以根据用户指定，有选择地提取列数据，如下所示：
...
在练习2.23中给出了一个列选择器的示例。不过，这里有一种实现方法：
...
换句话说，“name”是第0列，“shares”是第3列。当你从文件中读取一行数据时，这些索引用于过滤它：
