...
编写一个名为 `chunk_into_n(lst, n)` 的 Python 函数，该函数接受一个列表 `lst` 和一个整数 `n` 作为输入，并返回一个由 `n` 个较小列表组成的列表，每个较小列表包含原始列表中数量相等的元素。如果原始列表不能被均匀地拆分为 `n` 个较小列表，则最后一个块应包含剩余的元素。

要解决此问题，你可以按照以下步骤进行：

1. 通过将原始列表的长度除以 `n` 并使用 `math.ceil()` 函数向上取整到最接近的整数，来计算每个块的大小。
