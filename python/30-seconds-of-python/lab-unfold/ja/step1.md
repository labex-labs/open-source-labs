...
- 1 つの引数 (`seed`) を受け取り、常に 2 つの要素を持つリスト ([`value`, `nextSeed`]) または終了するための `False` を返すイテレータ関数 `fn`。
- 初期のシード値 `seed`。
