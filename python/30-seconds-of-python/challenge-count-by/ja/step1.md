...
最初の例では、関数 `floor` がリスト `[6.1, 4.2, 6.3]` の要素をグループ化するために使用されます。結果の辞書には2つのキーがあります。`6` と `4` で、それぞれの値は `2` と `1` です。

2番目の例では、関数 `len` がリスト `['one', 'two', 'three']` の要素をグループ化するために使用されます。結果の辞書には2つのキーがあります。`3` と `5` で、それぞれの値は `2` と `1` です。
