...
Comencemos nuestro viaje en los módulos de Python creando uno simple. En Python, un módulo es esencialmente un archivo con extensión `.py` que contiene código de Python. Piénsalo como un contenedor donde puedes agrupar funciones, clases y variables relacionadas. Esto hace que tu código esté más organizado y sea más fácil de manejar, especialmente a medida que tus proyectos aumentan de tamaño.
...
- La función `foo()` está definida para imprimir el valor de la variable global `x`. Las funciones son bloques de código reutilizables que realizan una tarea específica.
...
- La instrucción `print('Loaded simplemod')` es una instrucción de secuencia de comandos. Se ejecutará tan pronto como se cargue el módulo, lo que nos ayuda a confirmar que el módulo se ha cargado correctamente.
...
- Una variable global `x` con el valor `42`. Esta variable se puede utilizar en todo el módulo e incluso acceder desde otros módulos si se importa correctamente.
...
- Una instrucción `print` que se ejecuta cuando se carga el módulo. Esta instrucción sirve como indicador visual de que el módulo se ha cargado correctamente en el entorno de Python.

La instrucción `print` al final nos ayudará a observar cuándo se carga el módulo, lo cual es importante para la depuración y la comprensión de cómo funcionan los módulos en Python.