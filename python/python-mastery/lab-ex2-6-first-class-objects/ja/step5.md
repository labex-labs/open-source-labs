...
このエクササイズでは、`read_csv_as_dicts()` と `read_csv_as_columns()` の2つの関数を書きました。これらの関数は、ユーザーに対して同じようにデータを提示します。たとえば：
...
実際、書いたCTAデータ分析コードではどちらの関数も使うことができます。しかし、内部的にはまったく異なることが起こっています。`read_csv_as_columns()` 関数は、データを異なる表現形式で格納しています。それは、Pythonのシーケンスプロトコル（マジックメソッド）に依存して、より有用な方法で情報を提示しています。
