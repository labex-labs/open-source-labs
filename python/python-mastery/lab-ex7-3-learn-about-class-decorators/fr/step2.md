...
3. Une fois que vous avez ajouté le code, enregistrez le fichier `structure.py`. L'enregistrement du fichier garantit que vos modifications sont conservées.

4. Maintenant, nous devons modifier notre fichier `stock.py` pour utiliser ce nouveau décorateur. Ouvrez le fichier `stock.py` dans votre éditeur.
...
6. Après avoir effectué ces modifications, nous devons vérifier que tout fonctionne toujours comme prévu. Exécutez à nouveau les tests en utilisant les commandes suivantes :
