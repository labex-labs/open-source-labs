...
また、`Plant`構造体のスライスを含む`Nesting`という名前の構造体も作成する必要があります。`Nesting`構造体は`nesting`という名前のXML要素にマッピングされ、`Plant`構造体は`<parent><child>...`の下にネストされる必要があります。

その後、`Plant`と`Nesting`構造体をXMLにマーシャリングし、XMLデータを`Plant`構造体にアンマーシャリングするコードを書く必要があります。
