...
We will then create a container in interactive mode (using -it flags) from this image and name it `c2`.

```bash
docker run --name c2 -it img1
```
...
```bash
docker inspect -f "{{ json .Mounts }}" c2
```
