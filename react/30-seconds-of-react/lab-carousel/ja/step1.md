...
4. `React.cloneElement()` を使ってキャルーセル項目をレンダリングし、`...rest` を使って任意の追加のpropsを渡し、各項目に計算された `className` を追加します。

CSSスタイルはキャルーセルとその項目のレイアウトを定義します。キャルーセルコンテナは `position: relative` で、項目はデフォルトで `position: absolute` かつ `visibility: hidden` になっています。項目がアクティブになると、`visible` クラスが付与され、`visibility` が `visible` に設定されます。
