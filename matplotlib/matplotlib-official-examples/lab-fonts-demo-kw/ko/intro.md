...
이 랩에서는 Matplotlib 에서 키워드 인수를 사용하여 글꼴 속성을 설정하는 방법을 배웁니다. 다양한 글꼴 패밀리 (font family), 스타일 (style), 변형 (variant), 굵기 (weight), 크기 (size) 를 탐색하여 텍스트의 모양을 사용자 정의합니다. Matplotlib 의 `fig.text()` 메서드를 사용하여 다양한 글꼴 옵션을 표시합니다.
