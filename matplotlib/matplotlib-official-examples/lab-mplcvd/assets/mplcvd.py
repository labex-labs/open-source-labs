...
def _setup_gtk(tb):
    ...
    image = Gtk.Image.new_from_gicon(
        Gio.Icon.new_for_string(str(Path(__file__).parent / "images/eye-symbolic.svg")),
    ...
    if Gtk.check_version(3, 6, 0) is None:
        ...
        action = Gio.SimpleAction.new_stateful(
            "cvdsim", GLib.VariantType("s"), GLib.Variant("s", "none")
        )
...
def _setup_qt(tb):
    ...
    before = next(
        (
            ...
            if isinstance(tb.widgetForAction(action), QtWidgets.QLabel)
        ),
        None,
    )
...
def _setup_wx(tb):
    ...
    idx = next(
        idx for idx in range(tb.ToolsCount) if tb.GetToolByPos(idx).IsStretchableSpace()
    )
