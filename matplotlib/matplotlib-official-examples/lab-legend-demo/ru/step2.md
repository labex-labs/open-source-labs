# Построить более сложные метки

В этом шаге мы построим более сложные метки.
...
# Создать легенду с несколькими столбцами и заголовком
leg = ax0.legend(loc="upper left", bbox_to_anchor=[0, 1],
                 ncols=2, shadow=True, title="Legend", fancybox=True)
leg.get_title().set_color("red")

# Создать график с несколькими линиями и маркерами
ax1.plot(x, x**2, label="multi\nline")
half_pi = np.linspace(0, np.pi / 2)
ax1.plot(np.sin(half_pi), np.cos(half_pi), label=r"$\frac{1}{2}\pi$")
ax1.plot(x, 2**(x**2), label="$2^{x^2}$")

# Создать легенду с тенью
ax1.legend(shadow=True, fancybox=True)
