# 各画像に独自のカラーバーと異なるカラーバー範囲を持つ 2x2 の画像のグリッドを作成する

最後のグリッドも、各画像に独自のカラーバーを持つ 2x2 の画像のグリッドになりますが、今回は各画像に異なるカラーバー範囲を使用します。各画像を描画する際に、`vmin`と`vmax`を使用してカラーバー範囲を設定します。
