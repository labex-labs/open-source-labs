...
Ahora que tenemos definidas las clases `Pad` y `Puck`, podemos pasar a definir la clase `Game`. Esta clase se encargará de manejar la lógica del juego y de dibujar el juego en la pantalla.
...
class Game:
    def __init__(self, ax):
        ...
        self.res = 100.0
        self.on = False
        self.inst = True    # mostrar instrucciones desde el principio
    ...
    def draw(self):
        ...
        # restaurar el fondo limpio
        self.canvas.restore_region(self.background)

        # mostrar las distracciones
        if self.distract:
        ...
        if self.on:
            ...
            for puck in self.pucks:
                if puck.update(self.pads):
                    # solo llegamos aquí si alguien marcó un gol
        ...
        # solo volver a dibujar el rectángulo de los ejes
        self.canvas.blit(self.ax.bbox)
        self.canvas.flush_events()
        if self.cnt == 50000:
            # solo para no excedernos
            print("...y has estado jugando demasiado tiempo!!!")
            plt.close()
