# Форматирование объектов Форматтера

В этом шаге мы будем использовать объекты `.Formatter` для форматирования делений. Мы создадим семь графиков, каждый из которых будет использовать другой форматтер.
...
# FuncFormatter можно использовать в качестве декоратора
...
# Фиксированный форматтер
setup(axs1[3], title="FixedFormatter(['A', 'B', 'C',...])")
# FixedFormatter следует использовать только вместе с FixedLocator.
# В противном случае нельзя быть уверенным, где окажутся метки.
