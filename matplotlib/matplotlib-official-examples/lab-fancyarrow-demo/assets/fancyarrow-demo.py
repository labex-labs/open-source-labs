...
axs = (
    plt.figure(figsize=(4 * ncol, 1 + nrow))
    .add_gridspec(1 + nrow, ncol, wspace=0.7, left=0.1, right=0.9, bottom=0, top=1)
    .subplots()
)
for ax in axs.flat:
    ax.set_axis_off()
...
for ax, (stylename, stylecls) in zip(axs[1:, :].T.flat, styles.items()):
    ...
    ax.text(
        ...
        0.5,
        re.sub(
            ", ", lambda m, c=itertools.count(1): m.group() if next(c) % n else "\n", s
        ),
        transform=ax.transAxes,
        horizontalalignment="left",
        verticalalignment="center",
