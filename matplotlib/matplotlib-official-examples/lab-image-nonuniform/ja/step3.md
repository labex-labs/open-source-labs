# サブプロットとNonUniformImageを作成する

ここでは、サブプロットを作成し、それぞれにNonUniformImageを追加します。4つのサブプロットを作成し、2つは「nearest」補間、2つは「bilinear」補間を使用します。補間キーワード引数は、画像を表示するために使用される補間の種類を定義します。
