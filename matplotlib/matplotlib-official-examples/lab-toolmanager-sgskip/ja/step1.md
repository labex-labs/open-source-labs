...
最初のステップは、`ToolManager` によって制御されるすべてのツールを一覧表示することです。これは、`ListTools` という名前のカスタム ツールを作成することで達成できます。`ListTools` クラスは `ToolBase` から継承されています。`ListTools` の `trigger()` メソッドは、すべての利用可能なツールの名前、説明、キーマップを表示します。
