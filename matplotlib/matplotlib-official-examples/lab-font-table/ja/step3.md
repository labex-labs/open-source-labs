...
このステップでは、`argparse`を使ってコマンドライン引数からフォントファイルへのパスを解析します。そして、フォントファイル内のすべてのグリフを表示するために`print_glyphs`を呼び出し、フォントの最初の255文字のフォントテーブルを描画するために`draw_font_table`を呼び出します。
