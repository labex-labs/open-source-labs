# 소개

Matplotlib 는 Python 에서 정적, 애니메이션, 대화형 시각화를 생성하는 데 사용되는 Python 라이브러리입니다. 데이터 시각화를 위한 강력한 도구이며, 과학 커뮤니티에서 널리 사용됩니다. 이 랩에서는 Matplotlib 플롯에서 확대된 영역을 만드는 방법을 배웁니다.

## VM 팁

VM 시작이 완료되면, 왼쪽 상단을 클릭하여 **Notebook** 탭으로 전환하여 실습을 위해 [Jupyter Notebook](https://support.labex.io/en/labex-vm/jupyter)에 접속하십시오.

때로는 Jupyter Notebook 이 로딩을 완료하는 데 몇 초 정도 기다려야 할 수 있습니다. Jupyter Notebook 의 제한으로 인해 작업의 유효성 검사는 자동화될 수 없습니다.

학습 중에 문제가 발생하면 Labby 에게 문의하십시오. 세션 후 피드백을 제공해주시면 문제를 즉시 해결해 드리겠습니다.
