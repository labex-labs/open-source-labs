...
def bullseye_plot(ax, data, seg_bold=None, cmap="viridis", norm=None):
    ...
    for start, stop, r_in, r_out in [
        ...
        ax.bar(
            ...
            color="none",
            edgecolor="k",
            linewidth=[4 if i + 1 in seg_bold else 2 for i in range(start, stop)],
        )
...
fig.colorbar(
    ...
    cax=axs[0].inset_axes([0, -0.15, 1, 0.1]),
...
fig.colorbar(
    ...
    cax=axs[1].inset_axes([0, -0.15, 1, 0.1]),
    orientation="horizontal",
...
fig.colorbar(
    ...
    cax=axs[2].inset_axes([0, -0.15, 1, 0.1]),
    extend="both",
