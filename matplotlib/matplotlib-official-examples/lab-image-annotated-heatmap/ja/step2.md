...
データと行および列のラベルを入力として受け取り、プロットをカスタマイズするために使用される引数を許容する関数を作成します。周囲の軸のスパインを非表示にし、セルを区切るための白い線のグリッドを作成します。ここでは、カラーバーも作成し、ヒートマップの上にラベルを配置して下に配置しないようにします。注釈は、ピクセル色とのコントラストを向上させるために、閾値に応じて異なる色になるようにします。
