...
我们将看看一个处理缓慢的请求如何影响对当前服务器实现发出的其他请求。清单 20-10 实现了对`/sleep`的请求处理，它带有一个模拟的缓慢响应，这会导致服务器在响应之前休眠五秒钟。
...
第一个分支\[2\]与清单 20-9 中的`if`块相同。第二个分支\[3\]匹配对`/sleep`的请求。当接收到该请求时，服务器将休眠五秒钟，然后再渲染成功的 HTML 页面。第三个分支\[4\]与清单 20-9 中的`else`块相同。
