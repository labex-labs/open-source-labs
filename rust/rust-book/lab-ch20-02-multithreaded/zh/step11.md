...
但我们还没有完全完成！在 `Worker` 中，传递给 `thread::spawn` 的闭包仍然只是*引用*通道的接收端。相反，我们需要闭包永远循环，向通道的接收端请求任务，并在收到任务时运行它。让我们对 `Worker::new` 进行清单 20-20 所示的更改。
...
> 注意：如果你同时在多个浏览器窗口中打开 _/sleep_，它们可能会以五秒的间隔依次加载。由于缓存原因，一些网页浏览器会顺序执行同一请求的多个实例。这个限制不是由我们的 Web 服务器造成的。
...
清单 20-21：使用 `while let` 的 `Worker::new` 的替代实现
...
清单 20-20 中使用 `let job = receiver.lock().unwrap().recv().unwrap();` 的代码之所以有效，是因为使用 `let` 时，等号右侧表达式中使用的任何临时值在 `let` 语句结束时会立即被丢弃。但是，`while let`（以及 `if let` 和 `match`）直到相关块结束才会丢弃临时值。在清单 20-21 中，锁在调用 `job()` 的整个过程中都被持有，这意味着其他 `Worker` 实例无法接收任务。
