...
对 `thread::sleep` 的调用会强制一个线程在短时间内停止执行，从而允许另一个线程运行。线程可能会轮流执行，但这并不能保证：这取决于你的操作系统如何调度线程。在这次运行中，主线程先打印，尽管派生线程中的打印语句在代码中排在前面。而且，即使我们告诉派生线程打印到 `i` 为 9，在主线程关闭之前它只打印到了 5。
