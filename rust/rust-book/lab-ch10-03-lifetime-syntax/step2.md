...
The outer scope declares a variable named `r` with no initial value \[1\], and the inner scope declares a variable named `x` with the initial value of `5` \[2\]. Inside the inner scope, we attempt to set the value of `r` as a reference to `x` \[3\]. Then the inner scope ends \[4\], and we attempt to print the value in `r` \[5\]. This code won't compile because the value that `r` is referring to has gone out of scope before we try to use it. Here is the error message:
