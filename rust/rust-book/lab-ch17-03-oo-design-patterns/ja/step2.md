...
さて、ライブラリの実装を始めましょう！コンテンツを保持するパブリックな`Post`構造体が必要だとわかっているので、まずは構造体の定義と、`Post`のインスタンスを作成する関連するパブリックな`new`関数から始めます。リスト 17-12 に示すようになります。また、`Post`のすべての状態オブジェクトが持たなければならない動作を定義するプライベートな`State`トレイトも作成します。
...
リスト 17-12: `Post`構造体と新しい`Post`インスタンスを作成する`new`関数、`State`トレイト、および`Draft`構造体の定義
