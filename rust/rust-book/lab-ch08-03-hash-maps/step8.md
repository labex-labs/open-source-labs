...
This code will print `{"world": 2, "hello": 1, "wonderful": 1}`. You might see the same key--value pairs printed in a different order: recall from "Accessing Values in a Hash Map" on page XX that iterating over a hash map happens in an arbitrary order.
