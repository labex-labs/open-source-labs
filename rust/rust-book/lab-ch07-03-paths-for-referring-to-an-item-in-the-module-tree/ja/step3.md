...
シェフが注文の誤りを修正し、直接お客様に届ける状況を表現するコードをリスト7-8に示します。`back_of_house` モジュールに定義された `fix_incorrect_order` 関数は、`super` から始まる `deliver_order` へのパスを指定することで、親モジュールに定義された `deliver_order` 関数を呼び出します。
...
リスト7-8: `super` から始まる相対パスを使用した関数の呼び出し
