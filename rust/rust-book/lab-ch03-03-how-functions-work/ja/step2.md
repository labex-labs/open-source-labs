...
`another_function`の宣言には、`x`という名前の 1 つのパラメータがあります。`x`の型は`i32`と指定されています。`another_function`に`5`を渡すと、`println!`マクロは、フォーマット文字列の中の`x`を含む波括弧のペアの場所に`5`を置きます。
...
この例では、2 つのパラメータを持つ`print_labeled_measurement`という名前の関数を作成しています。最初のパラメータは`value`といい、`i32`型です。2 番目は`unit_label`といい、型は`char`です。その後、関数は`value`と`unit_label`の両方を含むテキストを表示します。
