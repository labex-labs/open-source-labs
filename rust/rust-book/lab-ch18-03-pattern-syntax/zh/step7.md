...
清单18-12展示了一个名为 `Point` 的结构体，它有两个字段 `x` 和 `y`，我们可以使用带有 `let` 语句的模式来拆解它。
...
这段代码创建了变量 `a` 和 `b`，它们与结构体 `p` 的 `x` 和 `y` 字段的值相匹配。这个例子表明，模式中的变量名不必与结构体的字段名匹配。然而，通常会将变量名与字段名匹配，以便更容易记住哪些变量来自哪些字段。由于这种常见用法，并且因为编写 `let Point { x: x, y: y } = p;` 包含大量重复，Rust 有一种用于匹配结构体字段的模式的简写形式：你只需要列出结构体字段的名称，从模式创建的变量将具有相同的名称。清单18-13的行为与清单18-12中的代码相同，但在 `let` 模式中创建的变量是 `x` 和 `y`，而不是 `a` 和 `b`。
