...
Обобщенное `Option<T>` заменяется конкретными определениями, созданными компилятором. Поскольку Rust компилирует обобщенный код в код, который задает тип в каждом экземпляре, мы не платим никаких накладных расходов во время выполнения за использование обобщений. Когда код выполняется, он работает так же, как будто мы вручную дублировали каждое определение. Процесс мономорфизма делает обобщения Rust极其эффективными во время выполнения.
