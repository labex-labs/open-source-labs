...
In a `for` loop, the value that directly follows the keyword `for` is a pattern. For example, in `for x in y`, the `x` is the pattern. Listing 18-3 demonstrates how to use a pattern in a `for` loop to _destructure_, or break apart, a tuple as part of the `for` loop.
