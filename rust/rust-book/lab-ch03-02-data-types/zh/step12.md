...
让我们看看如果你试图访问超出数组末尾的元素会发生什么。假设你运行这段类似于第2章猜数字游戏的代码，从用户那里获取一个数组索引：
...
这是 Rust 内存安全原则起作用的一个例子。在许多低级语言中，不会进行这种检查，当你提供一个错误的索引时，可能会访问无效内存。Rust 通过立即退出而不是允许内存访问并继续来保护你免受这种错误的影响。第9章将讨论更多关于 Rust 的错误处理，以及如何编写既不会 `panic` 也不会允许无效内存访问的可读、安全的代码。
