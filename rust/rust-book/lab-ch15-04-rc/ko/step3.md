...
참조 카운트가 변경되는 프로그램의 각 지점에서 `Rc::strong_count` 함수를 호출하여 얻는 참조 카운트를 출력합니다. 이 함수는 `count` 대신 `strong_count`로 명명되었습니다. `Rc<T>` 타입에는 또한 `weak_count`가 있기 때문입니다. "Weak`<T>`{=html}를 사용하여 참조 사이클 방지"에서 `weak_count`가 무엇에 사용되는지 살펴보겠습니다.
