...
`next` を呼び出すメソッドは、反復子を使い果たすために呼び出されるため、_消費アダプタ_ と呼ばれます。その 1 つの例が `sum` メソッドで、このメソッドは反復子の所有権を取得し、`next` を繰り返し呼び出すことで要素を反復処理し、したがって反復子を消費します。反復処理を行う際に、各要素を累積和に加え、反復処理が完了したときに合計値を返します。リスト 13-13 には、`sum` メソッドの使用例を示すテストがあります。
