...
Также вам понадобится _линкер_, который — это программа, которую Rust использует для объединения собранных им выходных данных в один файл.很可能您已经有一个了。如果遇到链接器错误，您应该安装一个C编译器，它通常会包含一个链接器。C编译器也很有用，因为一些常见的Rust包依赖于C代码，并且需要一个C编译器。
