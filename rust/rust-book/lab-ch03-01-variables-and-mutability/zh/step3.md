...
正如你在第 2 章的猜数字游戏教程中看到的，你可以声明一个与先前变量同名的新变量。Rust 开发者说第一个变量被第二个变量**遮蔽**了，这意味着当你使用变量名时，编译器看到的是第二个变量。实际上，第二个变量遮蔽了第一个变量，将变量名的任何使用都归为自身，直到它自己被遮蔽或作用域结束。我们可以通过使用相同的变量名并重复使用 `let` 关键字来遮蔽一个变量，如下所示：
