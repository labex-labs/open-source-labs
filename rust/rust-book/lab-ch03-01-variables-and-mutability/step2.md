...
First, you aren't allowed to use `mut` with constants. Constants aren't just immutable by default---they're always immutable. You declare constants using the `const` keyword instead of the `let` keyword, and the type of the value _must_ be annotated. We'll cover types and type annotations in "Data Types", so don't worry about the details right now. Just know that you must always annotate the type.
