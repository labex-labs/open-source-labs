...
第一行是**状态行**，包含响应中使用的 HTTP 版本、总结请求结果的数字状态码以及提供状态码文本描述的原因短语。在 CRLF 序列之后是任何头部信息、另一个 CRLF 序列以及响应的主体。
...
状态码 200 是标准的成功响应。这段文本是一个简短的成功 HTTP 响应。让我们将其写入流中作为对成功请求的响应！在`handle_connection`函数中，删除打印请求数据的`println!`，并用清单 20-3 中的代码替换它。
...
有了这些更改后，让我们运行代码并发出请求。我们不再向终端打印任何数据，所以除了 Cargo 的输出外我们不会看到任何其他输出。当你在 Web 浏览器中加载`127.0.0.1:7878`时，你应该会得到一个空白页面而不是错误页面。你刚刚手动编写了接收 HTTP 请求并发送响应的代码！
