...
Rustは、メタプログラミングを可能にする強力なマクロシステムを提供しています。前の章で見たように、マクロは関数のように見えますが、その名前の末尾にバング `!` が付きます。ただし、関数呼び出しを生成する代わりに、マクロはプログラムの残りの部分と一緒にコンパイルされるソースコードに展開されます。ただし、Cや他の言語のマクロとは異なり、Rustのマクロは文字列の事前処理ではなく抽象構文木に展開されるため、予期しない優先順位のバグが発生しません。
