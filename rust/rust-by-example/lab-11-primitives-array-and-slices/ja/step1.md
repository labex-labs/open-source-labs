...
スライスは配列に似ていますが、その長さはコンパイル時には既知ではありません。代わりに、スライスは2つのワードのオブジェクトです。最初のワードはデータへのポインタで、2番目のワードはスライスの長さです。ワードサイズは `usize` と同じで、プロセッサアーキテクチャによって決まります。たとえば、x86-64 では 64 ビットです。スライスは、配列の一部を借用するために使用でき、型シグネチャは `&[T]` です。
