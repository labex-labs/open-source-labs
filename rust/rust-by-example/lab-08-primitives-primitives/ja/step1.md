...
Rust は、幅広い「プリミティブ型」にアクセスできます。サンプルには以下が含まれます。
...
- 浮動小数点数：`f32`, `f64`
- `char` Unicode スカラー値、たとえば `'a'`, `'α'` および `'∞'` (それぞれ 4 バイト)
...
変数には常に型注釈を行うことができます。数値には、サフィックスまたはデフォルト値を使って追加的に注釈を行うことができます。整数はデフォルトで `i32`、浮動小数点数は `f64` です。Rust はコンテキストから型を推論することもできます。
...
fn main() {
    ...
    // 型はコンテキストからも推論できます。
    let mut inferred_type = 12; // 型 i64 は別の行から推論されます。
