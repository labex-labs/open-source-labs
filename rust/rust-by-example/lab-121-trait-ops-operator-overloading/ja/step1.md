...
Rustでは、多くの演算子をトレイトを通じてオーバーロードできます。つまり、一部の演算子は入力引数に基づいて異なるタスクを実行するために使用できます。これは、演算子がメソッド呼び出しのシンタックス・シュガーであるため可能です。たとえば、`a + b`の`+`演算子は`add`メソッドを呼び出します（`a.add(b)`のように）。この`add`メソッドは`Add`トレイトの一部です。したがって、`+`演算子は`Add`トレイトの実装者によって使用できます。
