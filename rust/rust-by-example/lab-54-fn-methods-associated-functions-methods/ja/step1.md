...
一部の関数は特定の型に関連付けられています。これらは2つの形式があります。関連付けられた関数とメソッドです。関連付けられた関数は、一般的に型に定義された関数であり、メソッドは型の特定のインスタンスに対して呼び出される関連付けられた関数です。
...
// `Pair` はリソースを所有しています。ヒープ上に割り当てられた2つの整数
