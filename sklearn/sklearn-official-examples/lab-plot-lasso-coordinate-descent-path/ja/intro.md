# はじめに

このチュートリアルでは、線形回帰に使用される手法であるラッソ（Lasso）とエラスティックネット（Elastic Net）について学びます。これらは座標降下法を用いて実装されます。また、ラッソとエラスティックネットを使って正則化パスを計算する方法と、matplotlibを使って結果を表示する方法についても学びます。

## VMのヒント

VMの起動が完了したら、左上隅をクリックして**ノートブック**タブに切り替え、Jupyter Notebookを使って練習しましょう。

Jupyter Notebookの読み込みには数秒かかる場合があります。Jupyter Notebookの制限により、操作の検証は自動化できません。

学習中に問題がある場合は、Labbyにお問い合わせください。セッション終了後にフィードバックを提供してください。すぐに問題を解決いたします。
