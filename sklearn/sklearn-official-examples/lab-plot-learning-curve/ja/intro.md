...
この実験の目的は、scikit - learnの`LearningCurveDisplay`クラスを使用して学習曲線をプロットする方法を示すことです。学習曲線は、訓練プロセス中にサンプルを追加することの影響を示します。我々は、手書き数字データセットを使って、単純ベイズ分類器とRBFカーネルを持つSVM分類器の学習曲線を分析します。さらに、これらの予測モデルの計算コストだけでなく、統計的な精度を見ることで、それらの拡張性を調べます。
