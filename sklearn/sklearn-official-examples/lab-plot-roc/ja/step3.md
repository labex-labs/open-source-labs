...
1対1（One-vs-One、OvO）の多クラス戦略は、クラスペアごとに1つの分類器を学習させることから成ります。`n_classes` \* (`n_classes` - 1) / 2個の分類器を学習させる必要があるため、この方法は通常、O(`n_classes` ^2)の計算量でOne-vs-Restよりも遅くなります。このステップでは、OvO多クラス戦略を使用してROC曲線をどのように計算するかを示します。
