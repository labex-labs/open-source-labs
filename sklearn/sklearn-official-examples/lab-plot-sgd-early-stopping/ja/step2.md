...
次のステップは、推定器と早期終了戦略を定義することです。scikit-learn の`SGDClassifier`モデルを使用します。3 つの異なる停止基準を定義します：停止基準なし、学習損失、検証スコア。`fit_and_score`関数を使用して、推定器をトレーニングセットに適合させ、両方のセットでスコア付けします。
